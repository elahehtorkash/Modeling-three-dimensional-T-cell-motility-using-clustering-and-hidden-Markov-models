...
#####    Comparing MSD and other measures of simulated trajectories with the true one. 
...
parameters {
    ...
    real<lower=0> mean_non_per_radius;

    real<lower=0> sigma_non_per_radius;

    real<lower=0> mean_per_radius;

    real<lower=0> sigma_per_radius;
   
    # real mean_transition_radius;

    # real<lower=0> sigma_transition_radius;
...
###  the persistent and non_persistent might be with different length
   ...
   for(n in 1:N_persist){
   	           	...
   	           	target += gamma_lpdf(radius_persist[n] | mean_per_radius, sigma_per_radius); 
   ...
   for(n in 1:N_non_persist){
   	           	...
   	           	target += gamma_lpdf(radius_non_persist[n] | mean_non_per_radius, sigma_non_per_radius); 
...
##########################################################################################################################################  STAN fit
                    ...
                    fit_radius_gamma <- stan(model_code = stanmodelcoderadius, model_name = "Radius",
                data = data_radius, iter = 1000, chains = 8, verbose = TRUE, control = list(adapt_delta = 0.9999, stepsize = 0.001, max_treedepth =15), cores=8)
                
                
                    fit_radius_gamma_ss <- summary(fit_radius_gamma) # fit_ss is a list


                    print(fit_radius_gamma_ss$summary)
